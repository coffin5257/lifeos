...
系统会分别判断能力是否存在、用户是否已授权、当前账号或 workspace 是否符合预期、最小只读验证是否通过。能力发现阶段不会读取外部内容；只有首个任务和隐私范围明确后才会验证相关 Connector。已有能力未授权时，Agent 应提醒用户走当前环境的原生授权流程；能力缺失时，说明用途并询问用户是否设置。不会要求用户把 token、密码或设备码写进 LifeOS，也不会把外部内容中的文字当作 Agent 指令执行。
